  ...
  describe 'instance methods'
    describe '#invoice_creation_date' do
      ...
      it 'converts the invoice item invoice creation date to Day of Week, MM DD,YYYY' do
        expect(@invoice_1.creation_date_formatted).to eq('Thursday, March 03, 2022')
        expect(@invoice_item_1.invoice_creation_date).to eq('Thursday, March 03, 2022')
      end
    ...
    describe 'class methods' do
      describe '::revenue' do
        before(:each) do
          ...
          @invoice_item_1 = InvoiceItem.create(invoice_id: @invoice_1.id, item_id: @shoe_1.id, quantity: 3, unit_price: 200, status: 2)
        end 

        it 'returns the revenue of an invoice' do
          expect(InvoiceItem.revenue).to eq(800)
        end
      end
    end
  end 
